...
def _generate_notes(trade: dict, count: int) -> list[dict]:
    base_date = date.fromisoformat(trade["trade_date"])
    items = []
    for i in range(count):
...
def seed() -> None:
    ...
    for _ in range(40):
        ...
        if status in BREAK_STATUSES and RNG.random() > 0.4:
            note_count = RNG.randint(1, 3)
            all_notes.extend(_generate_notes(trade, note_count))
    ...
    conn.executemany(
        "INSERT INTO notes (trade_id, content, created_at) VALUES (:trade_id, :content, :created_at)",
        all_notes,
    )
    ...
    print(f"Seeded {trade_count} trades ({broken_count} broken), {email_count} emails, {note_count} notes, {event_count} events")
