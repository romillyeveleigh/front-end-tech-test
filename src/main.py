...
from .database import create_note, get_all_trades, get_db, get_trade_by_id
from .models import (
    Note,
    NoteCreate,
    Trade,
...
def get_trade(trade_id: uuid.UUID) -> TradeDetail:
    """Get a single trade with its emails and notes."""
    with _db() as conn:
...
@app.post(
    "/api/trades/{trade_id}/notes",
    response_model=Note,
    status_code=201,
)
def submit_note(trade_id: uuid.UUID, body: NoteCreate) -> Note:
    """Submit a note for a trade in a break status.

    Only trades with a break status (dispute) accept notes.
    """
    with _db() as conn:
        try:
            return create_note(conn, trade_id=trade_id, content=body.content)
...
_CANNED_RESPONSES = [
    ...
    (
        "I notice this trade has multiple notes already, which suggests it's been "
        ...
        "all communication attempts are documented in the notes trail."
    ),
]
