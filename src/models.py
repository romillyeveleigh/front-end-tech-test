...
class Note(BaseModel):
    """A user-submitted note on a trade."""

    id: int
    trade_id: uuid.UUID
    content: Annotated[str, Field(description="The note text.")]
    created_at: datetime
...
class NoteCreate(BaseModel):
    ...
    content: Annotated[str, Field(min_length=1, max_length=500, description="The note text.")]
...
class EventLog(BaseModel):
    ...
    action: Annotated[str, Field(description="Short label for the action, e.g. 'status_change', 'note_added'.")]
...
class Trade(BaseModel):
    ...
    product_type: str
    has_notes: Annotated[bool, Field(description="Whether any notes have been submitted for this trade.")]


class TradeDetail(Trade):
    """Full trade representation including emails and notes."""
