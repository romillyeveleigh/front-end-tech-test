...
def _row_to_trade(row: sqlite3.Row, *, has_notes: bool) -> Trade:
    return Trade(
        id=uuid.UUID(row["id"]),
...
def get_all_trades(
    ...
    *,
    ...
    query = """
        SELECT t.*,
               EXISTS(SELECT 1 FROM notes n WHERE n.trade_id = t.id) AS has_notes
        FROM trades t
    """
    params: list[Any] = []
    ...
    return [_row_to_trade(r, has_notes=bool(r["has_notes"])) for r in rows]


def get_trade_by_id(conn: sqlite3.Connection, trade_id: uuid.UUID) -> TradeDetail | None:
    """Return a single trade with its emails and notes, or None if not found."""
    row = conn.execute(
        """
        SELECT t.*,
               EXISTS(SELECT 1 FROM notes n WHERE n.trade_id = t.id) AS has_notes
...
def create_note(
    ...
    *,
    ...
    """Insert a note for a trade and return the created record.
    ...
    if status not in BREAK_STATUSES:
        raise ValueError(
            f"Notes can only be submitted for trades in a broken status "
            f"({', '.join(s.value for s in BREAK_STATUSES)}), but this trade is '{status.value}'"
        )

    now = datetime.now(tz=timezone.utc).isoformat()
    cursor = conn.execute(
        "INSERT INTO notes (trade_id, content, created_at) VALUES (?, ?, ?)",
